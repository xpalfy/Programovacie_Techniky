...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    // TODO
    list<string> translated_sentence;
    for(const string& word:sentence){
        try{
            translated_sentence.push_back(translator.at(word));
        }
        catch (...){
            translated_sentence.push_back("?");
        }
    }
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    // TODO
    stack<char> zasobnik;
    for(basic_string<char>::size_type i=0;i<mathExpression.size();i++){
